...
- バックエンドで Bearer トークンを検証する
- Cosmos DB / Blob Storage を使う前提の構成を用意する
- Zod で API 入力値を検証する
...
├── /*      → React SPA
└── Auth    → MSAL.js (frontend) → Bearer Token → JWT 検証 (backend)
```
...
| データ取得 | SWR |
| 認証 | `@azure/msal-browser`, `@azure/msal-react`, `jose` |
| バリデーション | Zod |
...
| `AZURE_CLIENT_ID` | フロントエンド / API で使うアプリケーション ID |
| `COSMOS_*` | Cosmos DB 接続設定 |
...
- `webapp/server/middleware/auth.ts` を route に付与する
- 認証後のユーザー情報は `c.get("userId")` / `c.get("userEmail")` で参照する
- フロント側は `webapp/client/lib/api-client.ts` が自動で Bearer トークンを付与する
...
4. `webapp/server/routes/users.ts` — API がどう受けるか
5. `webapp/server/services/user.service.ts` — 業務処理の置き場所
6. `webapp/server/repositories/user.repository.ts` — DB 保存の実装
